...
describe('Testing the Layout of IssuersPage', () => {
  test('Check if the layout is matching with the snapshots', () => {
    mockUseFetchHook.mockReturnValue({ state: 'DONE', fetchRequest: jest.fn() });
    const { asFragment } = renderWithRouter(<IssuersPage />);
    expect(asFragment()).toMatchSnapshot();
  });
});

describe('Testing the Functionality of IssuersPage', () => {
  beforeEach(() => {
    mockusei18n();
