...
describe("Testing the Layouts of ItemBox", () => {
    test('Check if the layout is matching with the snapshots', () => {
        const {asFragment} = renderWithProvider(<ItemBox index={1} url={"/"} title={"TitleOfItemBox"} onClick={clickHandler} testIdSuffix='test-item-box'/>);
...
describe("Testing the Functionality ItemBox", () => {
    test('Check if content is rendered properly', () => {
        renderWithProvider(<ItemBox index={1} url={"/"} title={"TitleOfItemBox"} onClick={clickHandler} testIdSuffix='test-item-box'/>);
